  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === '/pin' && request.method === 'POST') {
        ...
        return j({ cid: body.cid, pinned: true, pinned_at: new Date().toISOString(), node: 'node-3' });
      }

      return j({ error: 'Not found', service: 'stratamesh-node-3' }, 404);
    } catch (err) {
