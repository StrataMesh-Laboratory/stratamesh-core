...
function solarTimes(date, lat, lon) {
  ...
  const eqOfTime =
    ...
    ((varY * Math.sin((2 * geomMeanLongSun * Math.PI) / 180) -
      ...
      180) /
    Math.PI;
  const tzMinutes = -date.getTimezoneOffset(); // WEST verão: +60; UTC: 0
...
const SYSCALLS = {
  registar_vertice: {
    holon: "dlt",
    descricao: "Anexa vértice à TRD (GDA) com selo PPC/CLP embutido",
  ...
  selo_temporal: {
    holon: "dlt",
    descricao: "Selo PPC/CLP (kernel temporal da TRD — não é camada)",
