...
function findStudent(id) {
  const u = String(id || "").toUpperCase();
  return (CATALOG.roster || []).find((s) => String(s.acb_id).toUpperCase() === u) || null;
}
function studentIds() { return (CATALOG.roster || []).map((s) => String(s.acb_id).toUpperCase()); }
...
function fluxTick(acbId, grade, prev) {
  const u = String(acbId || "").toUpperCase();
  const students = studentIds();
  const isStudent = !!u && students.indexOf(u) >= 0;
  const violations = [];
...
export default {
  async fetch(request, env) {
    ...
    if (path === "/v1/cognition/install" && request.method === "POST") {
      ...
      const student = findStudent(body.acb_id);
      if (!student) return json({ ok: false, error: "not_a_student" }, 403);
      const acb_id = student.acb_id;
    ...
    if (path === "/v1/debug/chat" && request.method === "POST") {
      ...
      const asked = String(body.acb_id || "ACB-ORCH-CMN-001");
      const student = findStudent(asked);
      const acb_id = (student && student.acb_id) || asked.toUpperCase();
      const fog = await probeFog();
      const prev = await readKv(env, "cognition:" + acb_id);
      const ctl = lobeController(prev);
