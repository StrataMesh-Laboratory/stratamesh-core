...
// stratamesh-fund.js
var VERSION = "0.4.6-grantor-brief";
var ORG = "StrataMesh-Laboratory";
var REPOS = [
...
var OPERATOR = {
  ...
  display_name: "Andr\xE9 M. Calhegas Morais",
  method: "eni_pagamentos",
  status: "active",
  widget_url: "https://calhegasmorais.pt/pagamentos",
  widget_note: "AMCM ENI payment portal \u2014 unique bank-transfer instruction (IBAN not published). Purpose: donation to the Calhegas Morais Node / StrataMesh project.",
  payment_intent_purpose: "donation",
  contact: "geral@eni.calhegasmorais.pt"
};
var SPONSORS_LOGIN = "amcmorais";
var SPONSORS_URL = "https://github.com/sponsors/amcmorais";
var SPONSORS_BUTTON = "https://github.com/sponsors/amcmorais/button";
var SPONSORS_CARD = "https://github.com/sponsors/amcmorais/card";
var SPONSORS_SETUP = "https://github.com/sponsors/accounts";
var SPONSORS_ORG = "StrataMesh-Laboratory";
var SPONSORS_ORG_URL = "https://github.com/sponsors/StrataMesh-Laboratory";
var CHALLENGE_REPOS = [
  { owner: ORG, name: "stratamesh-impact-fund" },
  { owner: ORG, name: "stratamesh-core" }
];
var CHALLENGE_LABEL = "impact-challenge";
var FUND_REPO = "stratamesh-impact-fund";
var POSTED_CHALLENGE_NUMBERS = [1, 2, 3, 4, 5, 6, 7, 8, 9];
var RANKING_NOTE = "No grantee deliveries until /accept + evidence PR.";
var SUMMARY_CACHE_MS = 30 * 60 * 1e3;
var RANKING_CACHE_MS = 10 * 60 * 1e3;
var PREPAID = [
  {
    ...
    description: "Grantor buys a prepaid product and ships to the contributor\u2019s confirmed public-domain email (registered at claim)."
  },
  {
    id: "wise_receive",
...
var SECURITY = {
  "Content-Security-Policy": "default-src 'self'; img-src 'self' data: https://avatars.githubusercontent.com https://github.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src https://fonts.gstatic.com data:; script-src 'self' 'unsafe-inline'; connect-src 'self' https://api.github.com; frame-src https://github.com; frame-ancestors 'none'",
...
function shell({ lang, path, title, active, body }) {
  ...
  const nav = `
    <nav class="nav" aria-label="primary">
      <a href="${homeHref}" class="${active === "home" ? "active" : ""}">${pt ? "In\xEDcio" : "Home"}</a>
      ...
      <span class="sep">\xB7</span>
      <a href="https://calhegasmorais.pt/">N\xF3</a>
      <span class="sep">\xB7</span>
...
<head>
  ...
  <meta name="description" content="${pt ? "Fundo StrataMesh \u2014 evid\xEAncia GitHub, GitHub Sponsors e grants por recipiente." : "StrataMesh fund \u2014 GitHub evidence, GitHub Sponsors, and per-recipient grants."}"/>
...
<body>
  <div class="wrap">
    <header class="top"><div class="top-inner">
      <a class="brand" href="${homeHref}" style="text-decoration:none"><strong>StrataMesh</strong> \xB7 Impact Fund</a>
      ${nav}
    </div></header>
    ${body}
    <footer>
      AMCM ENI \xB7 <a href="https://github.com/StrataMesh-Laboratory">StrataMesh Laboratory</a> \xB7 lab<br/>
      <div class="mono">stratamesh-fund ${VERSION} \xB7 GitHub evidence \xB7 no STRATA in V0</div>
    </footer>
  </div>
</body>
</html>`;
...
async function kvGetFresh(env, key, maxAgeMs) {
  ...
  if (!t || Date.now() - t > maxAgeMs)
    return null;
  return v;
}
__name(kvGetFresh, "kvGetFresh");
...
__name(kvListClaims, "kvListClaims");
...
async function sponsorsStatus(env) {
  ...
  return {
    ...
    organization: {
      ...
      legal_umbrella: "StrataMesh Laboratory exists under AMCM ENI (same payout/tax profile as the operator contributor @amcmorais). Org Sponsors uses the ENI umbrella; awaiting GitHub staff approval to go public.",
      note: orgLive ? "Organisation Sponsors is public." : orgPending ? "Organisation profile submitted \u2014 pending GitHub staff approval (not public yet). Same AMCM ENI account data as @amcmorais. Use @amcmorais Sponsors or ENI /pagamentos until live." : "Org Sponsors not submitted \u2014 enable at github.com/sponsors/accounts."
    },
    ...
    note: orgLive && userLive ? "Sponsors public for @amcmorais and StrataMesh-Laboratory (AMCM ENI umbrella)." : userLive && orgPending ? "Sponsors public for @amcmorais. StrataMesh-Laboratory listing submitted \u2014 pending GitHub staff approval (isPublic:false). Legal/payout umbrella: AMCM ENI." : userLive ? "Sponsors public for @amcmorais (AMCM ENI operator). Org listing not public yet." : "No public Sponsors listing \u2014 use ENI /pagamentos."
  };
}
__name(sponsorsStatus, "sponsorsStatus");
function classifyEnvelope({ budget_hint, funded }) {
  const raw = String(budget_hint || "").trim();
  const fundedTrue = funded === true;
  const looksUnfunded = !raw || raw === "\u2014" || raw === "-" || /^0(\b|[^\d]|$)/.test(raw) || /no current funding|unfunded|sem financiamento|n[aã]o depositado|not deposited|not treasury/i.test(raw);
  const bareNumber = /^\d+([.,]\d+)?$/.test(raw);
  const out = { funded: false, treasury: false };
  if (fundedTrue && raw && !looksUnfunded) {
    out.funded = true;
    out.budget_hint = raw;
    out.budget_display_pt = raw + " \xB7 envelope (n\xE3o tesouraria)";
    out.budget_display_en = raw + " \xB7 envelope (not treasury)";
    return out;
  }
  if (bareNumber && !fundedTrue) {
    out.budget_hint = "0 \u2014 no current funding yet";
    out.budget_display_pt = "0 \xB7 sem financiamento";
    out.budget_display_en = "0 \xB7 no current funding yet";
  ...
  out.budget_hint = raw;
  out.budget_display_pt = "proposto \xB7 n\xE3o depositado";
  out.budget_display_en = "proposed \xB7 not deposited";
  return out;
}
__name(classifyEnvelope, "classifyEnvelope");
function parseChallengeBody(body) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inline) {
      ...
      if (/^(true|false|yes|no|0|1)$/.test(v))
        funded = /^(true|yes|1)$/.test(v);
    }
  }
  const metrics = [];
  for (const line of lines) {
    ...
    if (m)
      metrics.push({ done: m[1].toLowerCase() === "x", text: m[2].trim() });
  }
...
__name(parseChallengeBody, "parseChallengeBody");
async function listChallenges(env) {
  const headers = ghHeaders(env);
  const items = [];
  for (const r of CHALLENGE_REPOS) {
    const url = "https://api.github.com/repos/" + r.owner + "/" + r.name + "/issues?state=open&labels=" + encodeURIComponent(CHALLENGE_LABEL) + "&per_page=50";
    try {
      ...
      for (const issue of list) {
        ...
        let phase = "open";
        if (labels.includes("challenge-delivered"))
          phase = "delivered";
        else if (labels.includes("challenge-accepted"))
          phase = "accepted";
        else if (labels.includes("challenge-open"))
          phase = "open";
  ...
  return {
    ...
    treasury: false,
    envelope_rule: "A budget figure is not treasury. V0 has no deposited/pledged/reserved EUR. Bare integers without Funded:true are withheld. Public default: 0 \u2014 no current funding yet.",
    principle: "The fund is stratified into open problems. Grantor chooses which challenge to fund. Grantee(s) accept and deliver against objective metrics agreed with the grantor. Rails: GitHub Sponsors + ENI /pagamentos. No envelope is cash until Funded:true + rail receipt."
  };
}
__name(listChallenges, "listChallenges");
async function discoverContributors(env) {
  ...
  for (const r of REPOS) {
    try {
      ...
      for (const c of list) {
        ...
        if (!prev.repositories.includes(r.name))
          prev.repositories.push(r.name);
        byId.set(c.id, prev);
      }
    } catch (_) {
      api_fail++;
  ...
  const contributors = [...byId.values()].map((c) => {
...
__name(discoverContributors, "discoverContributors");
function isAcceptComment(body) {
  return /^\s*\/accept\b/i.test(String(body || ""));
}
__name(isAcceptComment, "isAcceptComment");
...
__name(challengeIssueUrl, "challengeIssueUrl");
...
__name(pushSubstance, "pushSubstance");
...
__name(dedupeSubstance, "dedupeSubstance");
async function fetchSubstance(env, login) {
  ...
  await Promise.all(
    REPOS.map(async function(r) {
      try {
        const url = "https://api.github.com/repos/" + r.owner + "/" + r.name + "/commits?author=" + encodeURIComponent(login) + "&per_page=3";
        const res = await fetch(url, { headers });
        if (!res.ok)
          return;
        const list = await res.json();
        ...
        for (let i = 0; i < list.length; i++) {
          ...
          if (i === 0 && c.sha) {
            try {
              const prUrl = "https://api.github.com/repos/" + r.owner + "/" + r.name + "/commits/" + c.sha + "/pulls";
  ...
  try {
    ...
    const url = "https://api.github.com/search/issues?q=" + encodeURIComponent(q) + "&per_page=5&sort=updated";
    const pack2 = await ghJson(env, url);
    const items = pack2.ok && pack2.data && Array.isArray(pack2.data.items) ? pack2.data.items : [];
  ...
  try {
    const url = "https://api.github.com/users/" + encodeURIComponent(login) + "/events/public?per_page=20";
    const pack2 = await ghJson(env, url);
    const events = pack2.ok && Array.isArray(pack2.data) ? pack2.data : [];
    for (let i = 0; i < events.length; i++) {
      const ev = events[i];
      const full = ev.repo && ev.repo.name ? ev.repo.name : "";
      if (full.indexOf(ORG + "/") !== 0)
        continue;
  ...
  const pack = {
    ...
    evidence_at: (/* @__PURE__ */ new Date()).toISOString(),
    substance
  };
  await kvPut(env, cacheKey, pack, Math.floor(SUMMARY_CACHE_MS / 1e3));
  return { substance, evidence_at: pack.evidence_at, cached: false };
}
__name(fetchSubstance, "fetchSubstance");
function interpretExecutive(opts) {
  ...
  if (isOperator) {
    en.push(
      "@" + login + " is the laboratory operator (claimed; payout rail ENI /pagamentos). Grantor of posted challenges, not a ranked grantee."
    );
    pt.push(
      "@" + login + " \xE9 o operador do laborat\xF3rio (reclamado; rail de desembolso ENI /pagamentos). Grantor dos desafios publicados, n\xE3o um grantee no ranking."
    );
  } else if (claimed) {
    ...
    pt.push("@" + login + " n\xE3o reclamou perfil neste fundo (sem rail de desembolso registado).");
  }
  en.push(
    "GitHub contributor totals (org repos in scope): " + n + " contribution" + (n === 1 ? "" : "s") + " across " + repos.length + " repositor" + (repos.length === 1 ? "y" : "ies") + " (" + repoList + "). These are GitHub\u2019s contributor counts, not an impact score."
  );
  pt.push(
    "Totais GitHub (repos no \xE2mbito): " + n + " contribui\xE7" + (n === 1 ? "\xE3o" : "\xF5es") + " em " + repos.length + " reposit\xF3rio" + (repos.length === 1 ? "" : "s") + " (" + repoList + "). S\xE3o contagens da API GitHub, n\xE3o um score de impacto."
  );
  if (cites.length) {
    en.push("Substance from public GitHub evidence (recent commit/PR/issue titles): " + cites.join("; ") + ".");
    pt.push(
      "Subst\xE2ncia a partir de evid\xEAncia GitHub p\xFAblica (t\xEDtulos recentes de commit/PR/issue): " + cites.join("; ") + "."
    );
  } else {
    en.push(
      "No recent public commit, pull request, or issue titles were returned by the GitHub API for @" + login + " on in-scope repositories. Evidence missing."
    );
    pt.push(
      "A API GitHub n\xE3o devolveu t\xEDtulos recentes de commit, pull request ou issue para @" + login + " nos reposit\xF3rios no \xE2mbito. Evid\xEAncia em falta."
    );
  }
  if (opened.length) {
    ...
    en.push(
      "Opened challenges " + nums + " on " + FUND_REPO + " as grantor. Opening a challenge is not grantee delivery and does not enter the delivery ranking."
    );
    pt.push(
      "Abriu os desafios " + nums + " em " + FUND_REPO + " como grantor. Abrir um desafio n\xE3o \xE9 entrega de grantee e n\xE3o entra no ranking de entregas."
    );
  }
  if (activity && activity.comments) {
    const iss = (activity.issues || []).map(function(i) {
      return "#" + i.number;
    }).filter(function(v, i, a) {
      return a.indexOf(v) === i;
    }).join(", ");
    en.push(
      "Challenge-issue activity (comments, not delivery): " + activity.comments + " comment(s) on " + (iss || "listed issues") + "."
    );
    pt.push(
      "Actividade em issues de desafio (coment\xE1rios, n\xE3o entrega): " + activity.comments + " coment\xE1rio(s) em " + (iss || "issues listados") + "."
    );
  } else {
    en.push("No comments on posted challenge issues #1\u2013#9 were found for @" + login + ".");
    pt.push("N\xE3o foram encontrados coment\xE1rios de @" + login + " nos desafios publicados #1\u2013#9.");
  }
  if (deliveries.length) {
    en.push(
      "Challenge deliveries recorded: " + deliveries.map(function(d) {
        return d.challenge + " score " + d.score;
      }).join("; ") + "."
    );
    pt.push(
      "Entregas de desafio registadas: " + deliveries.map(function(d) {
        return d.challenge + " score " + d.score;
      }).join("; ") + "."
    );
  } else {
    en.push("No grantee delivery on posted challenges (requires /accept + evidence PR).");
    pt.push("Nenhuma entrega de grantee nos desafios publicados (exige /accept + PR de evid\xEAncia).");
  }
  en.push(
    "This brief interprets GitHub evidence for grantors. It is not a STRATA score, not treasury, and not a measure of euro value."
  );
  pt.push(
    "Este resumo interpreta evid\xEAncia GitHub para grantors. N\xE3o \xE9 um score STRATA, n\xE3o \xE9 tesouraria, nem uma medida de valor em euros."
  );
  return { executive_en: en.join(" "), executive_pt: pt.join(" ") };
}
__name(interpretExecutive, "interpretExecutive");
...
__name(extractLinkedPrs, "extractLinkedPrs");
async function fetchIssueComments(env, number) {
  const url = "https://api.github.com/repos/" + ORG + "/" + FUND_REPO + "/issues/" + number + "/comments?per_page=100";
  const pack = await ghJson(env, url);
  return pack.ok && Array.isArray(pack.data) ? pack.data : [];
}
__name(fetchIssueComments, "fetchIssueComments");
async function fetchIssueTimeline(env, number) {
  const url = "https://api.github.com/repos/" + ORG + "/" + FUND_REPO + "/issues/" + number + "/timeline?per_page=100";
  const pack = await ghJson(env, url);
  return pack.ok && Array.isArray(pack.data) ? pack.data : [];
}
__name(fetchIssueTimeline, "fetchIssueTimeline");
async function buildRanking(env, prefetched) {
  ...
  for (let i = 0; i < listed.length; i++) {
    const c = listed[i];
    if (c.repo && String(c.repo).indexOf(FUND_REPO) !== -1)
      byNumber[c.number] = c;
  }
  const packs = await Promise.all(
    POSTED_CHALLENGE_NUMBERS.map(async function(n) {
      ...
      const labeled = meta0 && (meta0.phase === "accepted" || meta0.phase === "delivered");
      const acceptedHere = labeled || comments.some(function(c) {
        return isAcceptComment(c.body);
      });
      const timeline = acceptedHere ? await fetchIssueTimeline(env, n) : [];
      return { n, comments, timeline };
    })
  );
  const deliveryRows = [];
  const activityByLogin = /* @__PURE__ */ new Map();
  let openCount = 0;
  for (let i = 0; i < packs.length; i++) {
    ...
    const accepted = meta.phase === "accepted" || meta.phase === "delivered" || (meta.labels || []).indexOf("challenge-accepted") !== -1 || (meta.labels || []).indexOf("challenge-delivered") !== -1 || acceptComments.length > 0;
    const grantees = /* @__PURE__ */ new Set();
    ...
    for (let a = 0; a < assignees.length; a++) {
      ...
      if (!login)
        continue;
      if (login === meta.user && acceptComments.length === 0)
        continue;
      grantees.add(login);
    }
    ...
    for (let c = 0; c < comments.length; c++) {
      ...
      if (!login)
        continue;
      if (comment.user && comment.user.type === "Bot")
        continue;
  ...
  const activity = Array.from(activityByLogin.values()).map(function(a) {
    return {
      login: a.login,
      comments: a.comments,
      score: a.comments,
  ...
  const payload = {
    ...
    note: RANKING_NOTE,
    rules: {
      deliveries: "accepted (/accept or challenge-accepted) + linked evidence PR + metrics_done. Opening a challenge as grantor does not count.",
      activity: "Comments on challenge issues excluding the original issue body. Labeled activity, not delivery.",
      no_ai_scores: true,
      no_strata: true,
      no_treasury: true
    }
  ...
  await kvPut(
    env,
    "gh:ranking:v1",
    { cached_at: payload.generated_at, payload },
    Math.floor(RANKING_CACHE_MS / 1e3)
  );
  return payload;
}
...
function openedChallengeNumbers(challenges, login) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (!c.repo || String(c.repo).indexOf(FUND_REPO) === -1)
      continue;
    if (POSTED_CHALLENGE_NUMBERS.indexOf(c.number) === -1)
      continue;
    out.push(c.number);
  }
...
__name(openedChallengeNumbers, "openedChallengeNumbers");
async function enrichContributor(env, contributor, ctx) {
  ...
  const activity = (ranking.activity || []).find(function(a) {
...
__name(enrichContributor, "enrichContributor");
async function contributorsWithSummaries(env) {
  ...
  const contributors = await Promise.all(
    (data.contributors || []).map(function(c) {
      return enrichContributor(env, c, { ranking, challenges: ch.challenges });
    })
  );
  return {
    contributors,
...
__name(contributorsWithSummaries, "contributorsWithSummaries");
function rankingSectionHtml(lang, ranking) {
  const pt = lang === "pt";
  const r = ranking || {};
  const deliveries = r.deliveries || [];
  const activity = r.activity || [];
  const delRows = deliveries.length ? deliveries.map(function(d) {
    const prs = (d.linked_prs || []).map(function(p) {
      return '<a href="' + esc(p.html_url) + '" rel="noopener">PR#' + esc(p.number) + "</a>";
    }).join(" \xB7 ");
    return '<tr><td><a href="/contributors/' + esc(d.login) + (pt ? "" : "?lang=en") + '">@' + esc(d.login) + '</a></td><td><a href="' + esc(d.html_url) + '" rel="noopener">' + esc(d.challenge) + '</a></td><td class="mono">' + esc(d.score) + '</td><td class="mono">' + (prs || "\u2014") + "</td></tr>";
  }).join("") : '<tr><td colspan="4" class="muted">' + (pt ? "Nenhuma entrega de grantee ainda. 0 /accept, 0 financed, m\xE9tricas por marcar nos 9 desafios. O ranking n\xE3o trata o autor do issue (grantor) como se tivesse resolvido o desafio." : "No grantee deliveries yet. 0 /accept, 0 funded, metrics unchecked on all 9 challenges. Ranking does not treat the issue author (grantor) as having solved the challenge.") + "</td></tr>";
  const actRows = activity.length ? activity.map(function(a) {
    const links = (a.issues || []).map(function(i) {
      return '<a href="' + esc(i.html_url) + '" rel="noopener">#' + esc(i.number) + "</a>";
    }).filter(function(v, i, arr) {
      return arr.indexOf(v) === i;
    }).join(" \xB7 ");
    const role = a.role === "grantor_operator" ? '<div class="muted">' + (pt ? "grantor / operador \u2014 actividade, n\xE3o entrega" : "grantor / operator \u2014 activity, not delivery") + "</div>" : "";
    return '<tr><td><a href="https://github.com/' + esc(a.login) + '" rel="noopener">@' + esc(a.login) + "</a>" + role + '</td><td class="mono">' + esc(a.comments) + '</td><td class="mono">' + (links || "\u2014") + "</td></tr>";
  }).join("") : '<tr><td colspan="3" class="muted">' + (pt ? "Sem coment\xE1rios nos desafios #1\u2013#9 (excluindo o corpo original)." : "No comments on challenges #1\u2013#9 (excluding the original issue body).") + "</td></tr>";
  return '<div class="section"><h2>Ranking</h2><p class="lead">' + (pt ? "Ranking s\xF3 de trabalho ligado aos desafios publicados (#1\u2013#9 neste reposit\xF3rio). Abrir um desafio como grantor n\xE3o conta como entrega de grantee. Sem scores de IA, sem STRATA, sem tesouraria." : "Ranking from work linked to posted challenges (#1\u2013#9 on this repository) only. Opening a challenge as grantor does not count as grantee delivery. No AI scores, no STRATA, no treasury.") + '</p><p class="note">' + esc(r.note || RANKING_NOTE) + "</p><h2>" + (pt ? "Entregas" : "Deliveries") + '</h2><div class="card" style="padding:0;overflow:auto"><table><thead><tr><th>' + (pt ? "Grantee" : "Grantee") + "</th><th>" + (pt ? "Desafio" : "Challenge") + "</th><th>Score</th><th>" + (pt ? "Evid\xEAncia" : "Evidence") + "</th></tr></thead><tbody>" + delRows + "</tbody></table></div><h2>" + (pt ? "Actividade (n\xE3o \xE9 entrega)" : "Activity (not delivery)") + '</h2><p class="muted">' + (pt ? "Coment\xE1rios nas issues dos desafios, excluindo o corpo original. Isto n\xE3o \xE9 ranking de entrega." : "Comments on challenge issues, excluding the original issue body. This is not a delivery ranking.") + '</p><div class="card" style="padding:0;overflow:auto"><table><thead><tr><th>' + (pt ? "Quem" : "Who") + "</th><th>" + (pt ? "Coment\xE1rios" : "Comments") + "</th><th>" + (pt ? "Evid\xEAncia" : "Evidence") + "</th></tr></thead><tbody>" + actRows + '</tbody></table></div><p class="mono muted"><a href="/api/v1/ranking">GET /api/v1/ranking</a></p></div>';
}
__name(rankingSectionHtml, "rankingSectionHtml");
...
__name(resolveLang, "resolveLang");
function sponsorsEmbedHtml(sp) {
  ...
  __name(dotClass, "dotClass");
  const desc = u.active ? "Support open contribution on StrataMesh Laboratory via GitHub Sponsors. Operator rail under AMCM ENI." : "GitHub Sponsors not public yet \u2014 use ENI /pagamentos for EUR grants.";
  let meta = '<span class="sp-dot ' + dotClass(u) + '"></span>@' + esc(u.login || SPONSORS_LOGIN) + " \xB7 " + statusLabel(u) + '&nbsp;&nbsp;&nbsp;<span class="sp-dot ' + dotClass(o) + '"></span>' + esc(SPONSORS_ORG) + " \xB7 " + statusLabel(o);
  if (o && o.pending) {
    meta += " \xB7 AMCM ENI umbrella (staff review)";
  }
  return '<div class="sponsor-panel"><div class="sponsor-panel-inner"><p class="sp-kicker">GitHub Sponsors</p><p class="sp-title">Fund the laboratory</p><p class="sp-desc">' + esc(desc) + '</p><div class="sp-actions"><a class="sp-btn" href="' + esc(url) + '" rel="noopener">Sponsor @' + esc(u.login || SPONSORS_LOGIN) + '</a><a class="sp-btn ghost" href="https://calhegasmorais.pt/pagamentos" rel="noopener">ENI /pagamentos</a>' + (o && o.active ? '<a class="sp-btn ghost" href="' + esc(o.url || SPONSORS_ORG_URL) + '" rel="noopener">Sponsor org</a>' : "") + '</div><p class="sp-meta">' + meta + "</p></div></div>";
}
__name(sponsorsEmbedHtml, "sponsorsEmbedHtml");
function homePage(lang, agg, sp) {
  ...
  const a = agg || {};
  const body = `
    <p class="kicker">${pt ? "V0 operacional \xB7 evid\xEAncia GitHub" : "Operational V0 \xB7 GitHub evidence"}</p>
    <h1>StrataMesh Impact Fund</h1>
    <p class="lead">${pt ? "Por agora o Impact Fund segue o GitHub Sponsors e cada recipiente individualmente. O fundo agrupado ser\xE1 libertado mais tarde. Evid\xEAncia p\xFAblica no GitHub." : "For now the Impact Fund follows GitHub Sponsors and each recipient individually. The pooled fund will be released later. Public GitHub evidence."}</p>
    <div class="actions">
    ...
    <div class="grid">
      ...
      <div class="stat-box"><div class="stat">${a.contributions != null ? a.contributions : "\u2026"}</div><div class="stat-label">${pt ? "contribui\xE7\xF5es GH" : "GH contributions"}</div></div>
      <div class="stat-box"><div class="stat">${a.repositories != null ? a.repositories : REPOS.length}</div><div class="stat-label">${pt ? "reposit\xF3rios" : "repositories"}</div></div>
      <div class="stat-box"><div class="stat">${a.claimed != null ? a.claimed : "\u2026"}</div><div class="stat-label">${pt ? "com payout" : "with payout"}</div></div>
    </div>
    <p class="note">${pt ? "N\xE3o h\xE1 pool de grants neste corte. Grantors usam GitHub Sponsors (@amcmorais) ou pagam cada contribuidor na conta registada. Fundo agrupado: mais tarde." : "No grant pool this cut. Grantors use GitHub Sponsors (@amcmorais) or pay each contributor on their registered account. Pooled fund later."}</p>
    <div class="section">
      <h2>${pt ? "Como funciona" : "How it works"}</h2>
      <ol class="steps">
        <li>GitHub \u2192 estat\xEDsticas por contribuidor nos repos StrataMesh-Laboratory</li>
        <li>${pt ? "Reclamar perfil (login p\xFAblico + email) ou operador via /pagamentos" : "Claim profile (public login + email) or operator via /pagamentos"}</li>
        <li>${pt ? "O grantor escolhe GitHub Sponsors ou paga cada recipiente; o fundo agrupado ainda n\xE3o est\xE1 aberto" : "The grantor chooses GitHub Sponsors or pays each recipient; the pooled fund is not open yet"}</li>
      </ol>
    </div>
    <div class="section">
      <h2>${pt ? "Reposit\xF3rios" : "Repositories"}</h2>
      <table>
    ...
    <div class="section">
      ...
      <p class="note">${pt ? "Desafios estratificam o fundo. Sponsors @amcmorais p\xFAblico. Org StrataMesh-Laboratory submetida \u2014 aguarda aprova\xE7\xE3o do staff GitHub (guarda-chuva AMCM ENI, mesmos dados de conta). /pagamentos dispon\xEDvel." : "Challenges stratify the fund. @amcmorais Sponsors is public. Org StrataMesh-Laboratory submitted \u2014 awaiting GitHub staff approval (AMCM ENI umbrella, same account data). /pagamentos available."}</p>
    </div>
    <div class="section">
      <h2>API</h2>
      <p class="mono muted">GET /api/v1/health \xB7 /contributors \xB7 /ranking \xB7 /repositories \xB7 /payout-methods \xB7 /claim (POST)</p>
    </div>`;
...
function contributorsPage(lang, data) {
  ...
  const rows = list.map((c) => {
    ...
    if (pay.status === "active")
      status = `<span class="pill ok">${esc(pay.method)}</span>`;
    else if (pay.status === "pending_review")
      status = `<span class="pill warn">pending</span>`;
    const link = pay.method === "eni_pagamentos" ? `<a href="${esc(pay.widget_url)}">/pagamentos</a>` : c.claimed ? `<span class="muted">${esc(pay.status || "claimed")}</span>` : `<a href="/claim?${pt ? "" : "lang=en&"}login=${encodeURIComponent(c.github_login)}">${pt ? "Reclamar" : "Claim"}</a>`;
  ...
  const body = `
    <p class="kicker">${pt ? "evid\xEAncia GitHub ao vivo" : "live GitHub evidence"}</p>
    <h1>${pt ? "Contribuidores" : "Contributors"}</h1>
    <p class="lead">${pt ? "Agregado a partir da API GitHub dos reposit\xF3rios da organiza\xE7\xE3o. O resumo de uma linha interpreta totais + subst\xE2ncia p\xFAblica \u2014 n\xE3o \xE9 um score de valor." : "Aggregated from the GitHub API across organization repositories. The one-line brief interprets totals + public substance \u2014 not a value score."}</p>
    <div class="card" style="padding:0;overflow:auto">
      <table>
        <thead><tr>
          <th>${pt ? "Contribuidor" : "Contributor"}</th>
          <th>${pt ? "Contribui\xE7\xF5es" : "Contributions"}</th>
    ...
    <p class="note mono">auth GH: ${data && data.aggregate && data.aggregate.authenticated ? "yes" : "no"} \xB7 api ${data && data.aggregate ? data.aggregate.github_api_ok + "/" + (data.aggregate.github_api_ok + data.aggregate.github_api_fail) : "?"}</p>
    <div class="actions">
      <a class="btn primary" href="/claim${enQ}">${pt ? "Reclamar perfil" : "Claim profile"}</a>
      <a class="btn" href="/api/v1/contributors">JSON</a>
    </div>`;
  return shell({ lang, path, title: pt ? "Contribuidores \xB7 Impact Fund" : "Contributors \xB7 Impact Fund", active: "contributors", body });
}
__name(contributorsPage, "contributorsPage");
function detailPage(lang, profile) {
  ...
  if (sum) {
    ...
    const items = (sum.substance || []).slice(0, 8).map((s) => `<li><span class="mono">${esc(s.kind)}</span> \xB7 <a href="${esc(s.html_url)}" rel="noopener">${esc(s.title)}</a> <span class="muted">${esc(s.repo)}</span></li>`).join("");
    briefBlock = `
      <div class="section">
        <h2>${pt ? "Resumo para grantors" : "Grantor brief"}</h2>
        <p>${esc(exec)}</p>
        ${items ? `<ul class="steps">${items}</ul>` : `<p class="muted">${pt ? "Sem t\xEDtulos de evid\xEAncia GitHub devolvidos pela API." : "No GitHub evidence titles returned by the API."}</p>`}
        <p class="mono muted">${pt ? "interpreta\xE7\xE3o de evid\xEAncia GitHub \xB7 n\xE3o \xE9 score de valor" : "GitHub-evidence interpretation \xB7 not a value score"} \xB7 ${esc(sum.evidence_at || "")}</p>
      </div>`;
  }
  let payBlock = "";
  if (pay.method === "eni_pagamentos") {
    ...
    payBlock = `
      ...
      <p class="mono">${esc(pay.method)} \xB7 ${esc(pay.status)}</p>
  ...
  const body = `
    ...
    <p class="muted"><a href="${esc(profile.profile_url || "https://github.com/" + login)}">GitHub</a>${profile.display_name ? " \xB7 " + esc(profile.display_name) : ""}</p>
    <div class="grid">
      <div class="stat-box"><div class="stat">${profile.contributions || 0}</div><div class="stat-label">${pt ? "contribui\xE7\xF5es" : "contributions"}</div></div>
    ...
    ${briefBlock}
    ${payBlock}`;
  return shell({ lang, path: `/contributors/${login}`, title: `@${login} \xB7 Impact Fund`, active: "contributors", body });
}
...
function challengesPage(lang, data, sponsors, ranking) {
  ...
  const rows = list.map((c) => {
    ...
    const pill = phase === "accepted" ? '<span class="pill warn">accepted</span>' : phase === "delivered" ? '<span class="pill ok">delivered</span>' : '<span class="pill">open</span>';
    const metrics = c.metrics_total > 0 ? c.metrics_done + "/" + c.metrics_total : "\u2014";
    return `<tr>
        <td><a href="${esc(c.html_url)}" rel="noopener">${esc(c.title)}</a>
          <div class="mono muted">${esc(c.id)}</div></td>
        <td>${pill}</td>
        <td class="mono">${esc((pt ? c.budget_display_pt : c.budget_display_en) || c.budget_hint || "0 \xB7 sem financiamento")}</td>
        <td class="mono">${metrics}</td>
        <td>${(c.assignees || []).map((a) => "@" + esc(a)).join(", ") || '<span class="muted">\u2014</span>'}</td>
      </tr>`;
  }).join("");
  const body = `
    <p class="kicker">${pt ? "problemas abertos \xB7 sem tesouraria V0" : "open problems \xB7 V0 has no treasury"}</p>
    <h1>${pt ? "Desafios de impacto" : "Impact challenges"}</h1>
    <p class="lead">${pt ? "Cada desafio permanece aberto at\xE9 um contribuidor ou equipa aceitar e servir a solu\xE7\xE3o contra m\xE9tricas objectivas acordadas entre grantor e grantees. O grantor escolhe qual projecto/desafio financiar." : "Each challenge stays open until a contributor or team accepts and delivers against objective metrics agreed between grantor and grantees. The grantor chooses which project/challenge to fund."}</p>
    <div class="grid">
    ...
    <p class="note">${pt ? "Nenhum valor na coluna Or\xE7amento \xE9 tesouraria. V0 n\xE3o tem EUR depositado, pledged ou reservado. Estado honesto at\xE9 um grantor depositar e o issue marcar Funded: true: 0 \xB7 sem financiamento." : "No figure in the Budget column is treasury. V0 has no deposited, pledged, or reserved EUR. Honest state until a grantor deposits and the issue marks Funded: true: 0 \xB7 no current funding yet."}</p>
    <div class="card" style="padding:0;overflow:auto">
      <table>
        <thead><tr>
          <th>${pt ? "Problema" : "Problem"}</th>
          <th>${pt ? "Fase" : "Phase"}</th>
          <th>${pt ? "Or\xE7amento" : "Budget"}</th>
          <th>${pt ? "M\xE9tricas" : "Metrics"}</th>
    ...
    <div class="section">
      <h2>${pt ? "Como funciona" : "How it works"}</h2>
      <ol class="steps">
        <li>${pt ? "Grantor abre um Issue com m\xE9tricas e financia via Sponsors ou /pagamentos (refer\xEAncia #issue)." : "Grantor opens an Issue with metrics and funds via Sponsors or /pagamentos (reference #issue)."}</li>
        <li>${pt ? "Contribuidor comenta /accept \u2014 desafio passa a aceite." : "Contributor comments /accept \u2014 challenge becomes accepted."}</li>
        <li>${pt ? "Entrega com evid\xEAncia; grantor confirma m\xE9tricas; issue fecha e o grant \xE9 libertado." : "Delivery with evidence; grantor confirms metrics; issue closes and the grant is released."}</li>
      </ol>
    </div>`;
  return shell({ lang, path, title: pt ? "Desafios \xB7 Impact Fund" : "Challenges \xB7 Impact Fund", active: "challenges", body });
}
__name(challengesPage, "challengesPage");
function claimPage(lang, prefillLogin) {
  const pt = lang === "pt";
  const path = pt ? "/claim" : "/claim?lang=en";
  const body = `
    <p class="kicker">${pt ? "registo de perfil \xB7 V0" : "profile registration \xB7 V0"}</p>
    <h1>${pt ? "Reclamar perfil" : "Claim profile"}</h1>
    <p class="lead">${pt ? "Regista o teu login GitHub p\xFAblico e um email de contacto (prefer\xEAncia: dom\xEDnio p\xFAblico). O grantor usa isto para desembolso. Sem OAuth nesta fase \u2014 verifica\xE7\xE3o por exist\xEAncia do utilizador na API GitHub." : "Register your public GitHub login and a contact email (prefer public-domain). The grantor uses this for payout. No OAuth in this phase \u2014 verified via GitHub user API existence."}</p>
    <form id="claimForm" class="section">
      ...
      <label>${pt ? "M\xE9todo de desembolso preferido" : "Preferred payout method"}</label>
    ...
    <p class="note">${pt ? "O operador AMCM usa /pagamentos \u2014 n\xE3o precisa deste formul\xE1rio." : "Operator AMCM uses /pagamentos \u2014 no need for this form."}</p>
    <script>
    document.getElementById('claimForm').addEventListener('submit', async function(e){
      ...
      try{
        ...
        var j=await r.json();
        if(r.ok && j.ok){ msg.className='okmsg'; msg.textContent=${pt ? "'Claim registado: '" : "'Claim registered: '"}+(j.github_login||'')+' \xB7 '+ (j.status||''); }
    ...
    <\/script>`;
  return shell({ lang, path, title: pt ? "Reclamar \xB7 Impact Fund" : "Claim \xB7 Impact Fund", active: "claim", body });
}
__name(claimPage, "claimPage");
async function handleClaim(env, body) {
  const login = String(body.github_login || body.login || "").trim().replace(/^@/, "");
  const email = String(body.email || "").trim().toLowerCase();
  const payout_method = body.payout_method === "prepaid_card" ? "prepaid_card" : "registered_account";
  if (!/^[A-Za-z0-9-]{1,39}$/.test(login))
    return { error: "invalid_github_login", status: 400 };
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
  ...
  if (res.status === 404)
    return { error: "github_user_not_found", status: 404 };
  if (!res.ok)
    return { error: "github_lookup_failed", status: 502, http: res.status };
  const user = await res.json();
  if (!env.FUND_KV)
    return { error: "kv_not_bound", status: 503 };
  const record = {
...
var stratamesh_fund_default = {
  async fetch(request, env) {
    ...
    if (path.startsWith("/api/v1/contributors/")) {
      ...
      if (!found)
        return json({ error: "not_found", github_login: key }, 404);
      return json(found);
    }
...
export {
  stratamesh_fund_default as default
};
//# sourceMappingURL=stratamesh-fund.js.map
