#!/usr/bin/env python3
"""Fog Node bootstrap wizard (macOS). Native pop-ups, then installer, then TUI.

1. Registered node id
2. 2FA to the operator email on file
3. GitHub PAT + Cloudflare API token (hidden)
4. Install Fog + workerd + stay-awake
5. Runtime UI

Secrets stay in ~/.config/stratamesh (0600). Never printed, never git.
"""
from __future__ import annotations

import json
import os
import subprocess
import sys
import urllib.error
import urllib.request
from pathlib import Path

AUTH = os.environ.get("FOG_AUTH_BASE", "https://calhegasmorais.pt/api/auth").rstrip("/")
HOME = Path.home()
FOG = Path(os.environ.get("STRATAMESH_HOME") or (HOME / "StrataMesh")) / "fog"
SECRETS = HOME / ".config" / "stratamesh"
REPO = FOG / "repo"
REPO_URL = "https://github.com/StrataMesh-Laboratory/stratamesh-core.git"


def as_lit(s: str) -> str:
    return json.dumps(s).replace("\\n", '" & return & "')


def osa(*lines: str) -> tuple[int, str, str]:
    r = subprocess.run(["osascript", *[x for ln in lines for x in ("-e", ln)]], capture_output=True, text=True)
    return r.returncode, (r.stdout or "").strip(), (r.stderr or "").strip()


def dialog(title: str, prompt: str, default: str = "", hidden: bool = False, extra: str = "") -> str:
    hid = " with hidden answer" if hidden else ""
    core = (
        "display dialog %s default answer %s buttons {\"Cancelar\",\"Continuar\"} "
        "default button \"Continuar\" with title %s%s"
        % (as_lit(prompt), as_lit(default), as_lit(title), hid)
    )
    scripts = [
        'tell application "Terminal"\nactivate\n%s\nend tell' % core,
        core,
    ]
    last_err = ""
    for sc in scripts:
        code, out, err = osa(sc)
        last_err = err or last_err
        if code == 0:
            marker = "text returned:"
            if marker in out:
                return out.split(marker, 1)[1]
            return out
        if "User canceled" in err or "não deu" in err.lower() or "-128" in err:
            raise SystemExit(1)
    print("Aviso: pop-up indisponível (%s). A usar o Terminal." % (last_err or "osascript"), file=sys.stderr)
    print()
    print("== %s ==" % title)
    print(prompt)
    if hidden:
        import getpass
        return getpass.getpass("→ ").strip()
    return input("→ [%s] " % default).strip() or default


def alert(title: str, prompt: str) -> None:
    core = "display dialog %s buttons {\"OK\"} default button 1 with title %s" % (as_lit(prompt), as_lit(title))
    for sc in ('tell application "Terminal"\nactivate\n%s\nend tell' % core, core):
        code, _, err = osa(sc)
        if code == 0:
            return
        if "User canceled" in err or "-128" in err:
            return
    print()
    print("== %s ==" % title)
    print(prompt)
    input("⏎  ")


def http_json(method: str, url: str, body: dict | None = None, token: str | None = None, timeout: float = 20) -> dict:
    data = None if body is None else json.dumps(body).encode()
    headers = {"Accept": "application/json", "User-Agent": "fog-bootstrap/8"}
    if body is not None:
        headers["Content-Type"] = "application/json"
    if token:
        headers["Authorization"] = "Bearer " + token
    req = urllib.request.Request(url, data=data, headers=headers, method=method)
    try:
        with urllib.request.urlopen(req, timeout=timeout) as r:
            raw = r.read().decode()
            return json.loads(raw) if raw else {}
    except urllib.error.HTTPError as e:
        raw = e.read().decode(errors="replace")
        try:
            j = json.loads(raw)
        except Exception:
            j = {"error": raw[:300], "http": e.code}
        j.setdefault("success", False)
        j["http"] = e.code
        return j
    except Exception as e:
        return {"success": False, "error": str(e)}


def write_secret(name: str, value: str) -> Path:
    SECRETS.mkdir(parents=True, exist_ok=True)
    os.chmod(SECRETS, 0o700)
    p = SECRETS / name
    p.write_text(value.strip() + "\n")
    os.chmod(p, 0o600)
    return p


def ensure_repo() -> None:
    FOG.mkdir(parents=True, exist_ok=True)
    if (REPO / ".git").exists():
        subprocess.call(["git", "-C", str(REPO), "pull", "--ff-only"])
        return
    rc = subprocess.call(["git", "clone", "--depth", "1", REPO_URL, str(REPO)])
    if rc != 0:
        alert("Fog", "Não foi possível clonar o repositório. Instale git (Xcode CLT) e volte a abrir o instalador.")
        raise SystemExit(2)


def main() -> int:
    ensure_repo()
    alert(
        "StrataMesh Fog",
        "Instalação do Nó Fog neste Mac.\n\n"
        "Vai precisar:\n"
        "1. o id de nó registado (ex. FOG-NODE-PT-CM-001)\n"
        "2. o código 2FA enviado ao e-mail do operador\n"
        "3. um token GitHub (ghp_…)\n"
        "4. um token Cloudflare (cfat_…)\n"
        "5. o token do túnel Cloudflare (opcional se já existir)\n\n"
        "As chaves ficam só neste Mac, em ~/.config/stratamesh.",
    )

    node_id = dialog("Nó Fog", "Id de nó registado:", "FOG-NODE-PT-CM-001").strip().upper()
    if not node_id:
        return 1
    ch = http_json("POST", AUTH + "/fog/bootstrap/challenge", {"node_id": node_id, "lang": "pt"})
    if not ch.get("success"):
        alert("Nó Fog", "Falha a pedir 2FA:\n" + str(ch.get("error") or ch)[:400])
        return 1
    masked = ch.get("operator_masked") or "operador@…"
    code = dialog(
        "2FA do operador",
        "Foi enviado um código de 6 dígitos para %s.\n\nIntroduza o código:" % masked,
        hidden=False,
    ).strip()
    vr = http_json(
        "POST",
        AUTH + "/fog/bootstrap/verify",
        {"node_id": node_id, "challenge": ch.get("challenge"), "code": code, "lang": "pt"},
    )
    if not vr.get("success"):
        alert("2FA", "Código recusado:\n" + str(vr.get("error") or vr)[:400])
        return 1
    write_secret("node.id", node_id)
    write_secret("bootstrap.token", vr.get("bootstrap_token") or "")

    gh = dialog(
        "GitHub",
        "Cole o Personal Access Token (ghp_…).\n"
        "github.com → Settings → Developer settings → Fine-grained or classic PAT.\n"
        "Permissões: repo (StrataMesh-Laboratory).",
        hidden=True,
    ).strip()
    if not gh.startswith("ghp_") and not gh.startswith("github_pat_"):
        alert("GitHub", "O token deve começar por ghp_ (ou github_pat_). Nada foi gravado.")
        return 1
    who = http_json("GET", "https://api.github.com/user", token=gh)
    if not who.get("login"):
        alert("GitHub", "Token recusado pela API GitHub.")
        return 1
    write_secret("github.pat", gh)
    write_secret("gh_pat", gh)

    cf = dialog(
        "Cloudflare",
        "Cole o API token Cloudflare (cfat_…).\n"
        "dash.cloudflare.com → My Profile → API Tokens.\n"
        "Workers, DNS, Account — o token de operador (god_api).",
        hidden=True,
    ).strip()
    if not cf.startswith("cfat_") and not cf.startswith("Bearer "):
        # allow other CF token forms but warn
        if len(cf) < 20:
            alert("Cloudflare", "Token demasiado curto. Nada foi gravado.")
            return 1
    write_secret("cloudflare.token", cf)
    write_secret("god_api", cf)

    tun_path = SECRETS / "tunnel.token"
    if not tun_path.is_file() or tun_path.stat().st_size < 20:
        tun = dialog(
            "Túnel Cloudflare",
            "Cole o token do named tunnel (macbook-server).\n"
            "Zero Trust → Tunnels → macbook-server → token.\n"
            "Já existir em disco: Cancelar e o instalador reutiliza.",
            hidden=True,
        ).strip()
        if tun:
            write_secret("tunnel.token", tun)

    write_secret("cf_account", "f3645fcb56675cf7250d8ba7358eb252")
    alert(
        "Instalar",
        "Identidade ok: %s\nOperador: %s\nGitHub: @%s\n\nA instalar Fog + workerd + stay-awake neste Mac."
        % (node_id, vr.get("operator_masked"), who.get("login")),
    )

    env = os.environ.copy()
    env["FOG_NODE_ID"] = node_id
    inst = REPO / "deploy/mac-fog/FogNodeInstaller.command"
    rc = subprocess.call(["bash", str(inst)], env=env)
    if rc != 0:
        alert("Instalação", "O instalador devolveu código %s. Veja o Terminal." % rc)
        return rc
    subprocess.call(["bash", str(REPO / "deploy/mac-fog/FogStayAwake.command"), "--no-tui"], env=env)
    subprocess.call(["bash", str(REPO / "deploy/mac-fog/build-apps.sh")], env=env)

    tui = FOG / "bin" / "fog-tui.py"
    if not tui.is_file():
        tui = REPO / "deploy/mac-fog/fog-tui.py"
    os.environ["FOG_HOME"] = str(FOG)
    os.environ["FOG_NODE_ID"] = node_id
    os.execvp("python3", ["python3", str(tui)])
    return 0


if __name__ == "__main__":
    try:
        raise SystemExit(main())
    except SystemExit:
        raise
    except Exception as e:
        try:
            alert("Fog", "Erro: " + str(e)[:500])
        except Exception:
            print("ERR", e, file=sys.stderr)
        raise SystemExit(1)
