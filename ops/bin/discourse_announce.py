...
TITLE = sys.argv[1] if len(sys.argv) > 1 else "v0.6.0-lab · Fog desk collegium"
BODY_PATH = sys.argv[2] if len(sys.argv) > 2 else "docs/DISCOURSE-v0.6.0-lab.md"
CAT = int(sys.argv[3]) if len(sys.argv) > 3 else 5

def read_body():
    if os.path.isfile(BODY_PATH):
        return open(BODY_PATH, encoding="utf-8").read()
    url = "https://raw.githubusercontent.com/StrataMesh-Laboratory/stratamesh-core/main/docs/DISCOURSE-v0.6.0-lab.md"
