...
`main_module` map: spa/orchestrator/`stratamesh-node-public` → `index.js`; status → `worker.js`; fund → `stratamesh-fund.js`; gossip → `stratamesh-gossip.js`; origin-archive → `stratamesh-origin-archive.js`.
