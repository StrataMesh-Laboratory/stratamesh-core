...
LANE_SPECS = {
    ...
    "lane-hermes": {
        ...
        "renewal": "host_cap",
        "renewal_note": "no CF daily clock; primary llama3.2:1b (light) + Qwen2.5 3b/7b OSS fallbacks each with own host_cap metabol_pace; need context≥65536; host RAM/CPU",
        "context_min": 65536,
...
PLATFORM_SPECS = {
    ...
    "ollama-llama32-1b": {
        "renewal": "host_cap",
        "quota": None,
        "unit": "host RAM/CPU + context tokens",
        "burn_rate": "light — 1.3GB weights; prefer ALLOW for desk coord; HOLD only when host_cap.over",
        "usage": "Hermes primary OSS model (Meta Llama 3.2 1B); context 131072; tools OK; free local",
        "context_min": 65536,
        "context_reported": 131072,
        "size_gb": 1.3,
        "role": "primary",
        "lane": "lane-hermes",
        "complement": "ollama-qwen25",
    },
    "ollama-qwen25-3b": {
        "renewal": "host_cap",
        "quota": None,
        "unit": "host RAM/CPU + context tokens",
        "burn_rate": "medium — ~2GB; complementary failover; pace STASIS sooner under RAM≥60% host row",
        "usage": "Hermes OSS fallback #1 (Qwen2.5 3B Apache-2.0); ≥64K + tools; pull via ollama",
        "context_min": 65536,
        "role": "fallback",
        "lane": "lane-hermes",
        "complements": "ollama-llama32-1b",
    },
    "ollama-qwen25-7b": {
        "renewal": "host_cap",
        "quota": None,
        "unit": "host RAM/CPU + context tokens",
        "burn_rate": "heavy — ~4.7GB; complementary only when llama+3b fail or deep coding; metabol HOLD when host pressure",
        "usage": "Hermes OSS fallback #2 (Qwen2.5 7B Apache-2.0); ≥64K + tools; slower burn",
        "context_min": 65536,
        "role": "fallback_heavy",
        "lane": "lane-hermes",
        "complements": "ollama-llama32-1b",
