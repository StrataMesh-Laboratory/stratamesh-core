# ACB volition — self-directed ends and means

**Worker:** `stratamesh-acb` · **version:** 5.4.0-volition

## Ontology
- ACBs **hold goals** (ends). `origin: self` vs `human_proposal` marks source of the statement, not ownership of will.
...
| `POST /acb/volition-cycle` | Population: each active ACB deliberates + one act |
