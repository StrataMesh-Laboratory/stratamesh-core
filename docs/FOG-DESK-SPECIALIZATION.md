...
|-------|----------------|--------------|--------------|
| **STRATAGROK** (`grok@`) | Eisenhower rank, meters, Git Data API lead, human gates, idle-rule, multi-surface fact register | Local Mac GUI click-storms, Fog TUI `g` | André (`g`/2FA); Assistants for scoped Acts |
...
| **CMN FOG ASSISTANT** | Fog/core origin: hops, Pages HTML, auth/mw (one prompt) | EDGE thread, Bot multitask | Bot feeds next Act; EDGE stays split |
| **CMN EDGE ASSISTANT** | EDGE consume-origin / edge surface (one prompt) | Fog/core mixed todos | Bot feeds next Act |
